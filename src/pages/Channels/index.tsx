...
export function Channels() {
  ...
  const fetchPageData = useCallback(async (options?: FetchPageDataOptions) => {
    ...
    try {
      const channelsPath = '/api/channels/accounts?mode=config';
    ...
    <div data-testid="channels-page" className="flex flex-col -m-6 dark:bg-background h-[calc(100vh-2.5rem)] overflow-hidden">
      <div className="w-full max-w-5xl mx-auto flex flex-col h-full p-10 pt-16">
        <div className="bajo-page-header mb-8 shrink-0 px-6 py-5">
          ...
          <div className="min-w-0">
            ...
            <p className="max-w-2xl text-[14px] font-medium leading-6 text-foreground/70">
              绑定 BajoSeek 机器人账号后，BajoClaw 会通过内置 OpenClaw 插件接收消息、调用模型和技能，并把回复发回用户。
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto pr-2 pb-10 min-h-0 -mr-2">
          ...
          {configuredGroups.length > 0 && (
            <div className="mb-8">
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="text-[12px] font-medium text-muted-foreground">用于接收 BajoSeek Bot 消息</span>
              </div>
                      ...
                      {group.accounts.map((account) => {
                        const displayName =
                          account.accountId === 'default' && account.name === account.accountId
                            ...
                            : account.name;
                        return (
                        <div key={`${group.channelType}-${account.accountId}`} className="min-h-[52px] rounded-md border border-black/10 bg-white/30 px-3 py-2 transition-colors hover:border-blue-500/25 dark:border-white/10 dark:bg-white/5">
          ...
          <div className="mb-8">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-xl font-bold text-foreground">添加 BajoSeek 通道</h2>
