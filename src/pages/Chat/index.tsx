...
function shouldHideChatError(error: string | null): boolean {
  return !!error && classifyHistoryStartupRetryError(error) !== null;
}
...
export function Chat() {
  ...
  const visibleError = shouldHideChatError(error) ? null : error;
  ...
  return (
    <div className={cn("relative flex min-h-0 flex-col -m-6 transition-colors duration-500 dark:bg-background")} style={{ height: 'calc(100vh - 2.75rem)' }}>
      {/* Toolbar */}
      <div className="flex shrink-0 items-center justify-between border-b border-black/10 px-6 py-3 dark:border-white/10">
        <div>
          ...
          <div className="text-[12px] font-medium text-foreground/55">通过 BajoSeek 连接用户消息，调用内置 OpenClaw 能力</div>
        </div>
        <ChatToolbar />
      </div>
  ...
  return (
    <div className="mx-auto flex h-[60vh] max-w-3xl flex-col items-center justify-center text-center">
      ...
      <p className="mb-7 max-w-xl text-[14px] font-medium leading-6 text-foreground/62">
        配置模型和 BajoSeek 机器人后，可直接在桌面端处理用户消息、调用技能并管理自动化任务。
      </p>
      ...
      <div className="flex flex-wrap items-center justify-center gap-2.5 max-w-lg w-full">
        {quickActions.map(({ key, label }) => (
          <button 
            key={key}
            className="rounded-md border border-black/10 bg-white/40 px-4 py-2 text-[13px] font-semibold text-foreground/70 transition-colors hover:bg-white/70 dark:border-white/10 dark:bg-white/10 dark:hover:bg-white/15"
          >
            {label}
          </button>
        ))}
