...
import { getLocalizedMarketplaceSkill, getLocalizedSkill, getSkillSearchText } from '@/lib/skill-localization';
...
function hasSavedSkillConfig(skill: Skill): boolean {
  const apiKey = typeof skill.config?.apiKey === 'string' ? skill.config.apiKey.trim() : '';
  const env = skill.config?.env;
  return Boolean(apiKey) || Boolean(env && typeof env === 'object' && Object.keys(env as Record<string, unknown>).length > 0);
}
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-8 py-10">
          <div className="bajo-panel mb-8 flex flex-col items-center px-5 py-6">
            ...
            <h2 className="text-[22px] font-bold text-foreground mb-3 text-center tracking-tight">
              {displaySkill.name}
            </h2>
...
export function Skills() {
  ...
  const filteredSkills = safeSkills.filter((skill) => {
    ...
    const matchesSearch =
      q.length === 0 ||
      getSkillSearchText(skill).includes(q);
    ...
    if (!a.isCore && b.isCore) return 1;
    return getLocalizedSkill(a).name.localeCompare(getLocalizedSkill(b).name, 'zh-CN');
  });
          ...
          <div className="mt-5 flex items-center gap-3">
            {hasInstalledSkills && (
              <button
                ...
                className="hover:bg-white/70 dark:hover:bg-white/10 transition-colors shrink-0 text-[13px] font-semibold px-4 h-8 rounded-md border border-black/10 dark:border-white/10 flex items-center justify-center text-foreground/80 hover:text-foreground bg-white/40 dark:bg-white/5"
              >
                  ...
                  <div className="flex items-start gap-4 flex-1 overflow-hidden pr-4">
                    ...
                    <div className="flex flex-col overflow-hidden">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="text-[15px] font-semibold text-foreground truncate">{displaySkill.name}</h3>
                        {skill.isCore ? (
      ...
      <Sheet open={installSheetOpen} onOpenChange={setInstallSheetOpen}>
        ...
        >
          <div className="px-7 py-6 border-b border-black/10 dark:border-white/10">
            <h2 className="text-[22px] font-bold text-foreground tracking-tight">{t('marketplace.installDialogTitle')}</h2>
            ...
            {!searching && searchResults.length > 0 && (
              <div className="flex flex-col gap-1">
                {searchResults.map((skill) => {
                  const displaySkill = getLocalizedMarketplaceSkill(skill);
                      ...
                      <div className="flex items-center gap-4 shrink-0" onClick={e => e.stopPropagation()}>
                        ...
                        {isInstalled ? (
                          ...
                          <Button
                            ...
                            disabled={isInstallLoading}
                            className="h-8 px-4 rounded-md shadow-none font-medium text-xs"
                          >
