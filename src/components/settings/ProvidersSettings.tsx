...
function AddProviderDialog({
  ...
  const handleValidate = async () => {
    ...
    if (!baseUrl.trim()) {
      setValidationError('请输入 Base URL');
    ...
    try {
      ...
      if (!result.valid) {
        setValidationError(result.error || '验证失败，请检查 Base URL 和 API Key');
        return;
      }
      setValidationSuccess('验证通过，可以添加模型');
    } finally {
      setValidating(false);
    }
  };
      ...
      <Card className="w-full max-w-2xl max-h-[90vh] flex flex-col rounded-lg border border-black/10 dark:border-white/10 shadow-2xl bg-[#f3f1e9] dark:bg-card overflow-hidden">
        <CardHeader className="relative shrink-0 border-b border-black/10 pb-4 dark:border-white/10">
          ...
          <Button
            ...
            variant="ghost"
            size="icon"
            className="absolute right-4 top-4 rounded-md h-8 w-8 -mr-2 -mt-2 text-muted-foreground hover:text-foreground hover:bg-black/5 dark:hover:bg-white/5"
            onClick={onClose}
          >
            <X className="h-4 w-4" />
          </Button>
          ...
          {false ? (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {availableTypes.map((type) => (
                ...
                >
                  <div className="h-12 w-12 mx-auto mb-3 flex items-center justify-center bg-black/5 dark:bg-white/5 rounded-xl shadow-sm border border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-6 bg-transparent p-0">
                {false && <div className="space-y-2.5">
                ...
                {typeInfo?.showBaseUrl && (
                  <div className="space-y-2.5">
                    <Label htmlFor="baseUrl" className={labelClasses}>Base URL</Label>
                    <Input
                ...
                {showModelIdField && (
                  <div className="space-y-2.5">
                    <Label htmlFor="modelId" className={labelClasses}>模型名称</Label>
                    <Input
              ...
              {(validationError || validationSuccess) && (
                <p className={cn(
                  ...
                  validationError ? 'text-red-500' : 'text-green-600 dark:text-green-400',
                )}>
                  {validationError || validationSuccess}
                </p>
              )}
              ...
              <div className="flex justify-end gap-3">
                <Button
                  data-testid="add-provider-validate-button"
                  variant="outline"
                  onClick={handleValidate}
                  className="rounded-md px-8 h-[42px] text-[13px] font-semibold shadow-sm"
                  disabled={validating || saving || !baseUrl.trim() || !apiKey.trim() || !modelId.trim()}
                >
                  {validating ? (
