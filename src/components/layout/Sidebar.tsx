...
function getDisplayAgentName(agentId: string, rawName?: string): string {
  const normalizedId = (agentId || '').trim();
  const normalizedName = (rawName || '').trim();
  if (normalizedId === 'main' || normalizedName.toLowerCase() === 'main agent') {
    return '主助手';
  }
...
export function Sidebar() {
  ...
  return (
    ...
    >
      {/* Top Header Toggle */}
      <div className={cn("flex items-center px-3 py-3", sidebarCollapsed ? "justify-center" : "justify-between")}>
        {!sidebarCollapsed && (
      ...
      {!sidebarCollapsed && sessions.length > 0 && (
        <div className="mt-4 flex-1 overflow-y-auto overflow-x-hidden px-3 pb-2 space-y-1">
          {sessionBuckets.map((bucket) => (
            bucket.sessions.length > 0 ? (
              <div key={bucket.key} className="pt-2">
                <div className="px-2.5 pb-1 text-[11px] font-semibold text-foreground/50 tracking-tight">
                  {bucket.label}
                </div>
                {bucket.sessions.map((s) => {
                  const agentId = getAgentIdFromSessionKey(s.key);
                  const agentName = getDisplayAgentName(agentId, agentNameById[agentId]);
                  return (
      ...
      <div className="mt-auto border-t border-black/10 p-3 dark:border-white/10">
        ...
        <Button
          data-testid="sidebar-open-openclaw-webui"
          variant="ghost"
          className={cn(
            'flex items-center gap-3 rounded-md px-2 py-2 h-auto text-[14px] font-semibold transition-colors w-full',
            'hover:border-blue-500/20 hover:bg-white/45 dark:hover:bg-white/10 text-foreground/78 border border-transparent',
        ...
        >
          ...
          {!sidebarCollapsed && (
            <>
              <span className="flex-1 text-left overflow-hidden text-ellipsis whitespace-nowrap">高级控制台</span>
