...
const IMAGE_CACHE_KEY = 'bajo-claw:image-cache';
...
function isBenignHistoryLoadFailure(error: unknown): boolean {
  return classifyHistoryStartupRetryError(error) !== null;
}
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  loadHistory: async (quiet = false) => {
    ...
    const loadPromise = (async () => {
      ...
      const applyLoadFailure = (errorMessage: string | null, suppressError = false) => {
        if (!isCurrentSession()) return;
        set((state) => {
          const hasMessages = state.messages.length > 0;
          return {
            loading: false,
            error: !quiet && errorMessage && !suppressError ? errorMessage : state.error,
      ...
      try {
        ...
        if (data) {
          ...
          const benignHistoryFailure = isBenignHistoryLoadFailure(lastError);
          if (isCurrentSession() && isInitialForegroundLoad && benignHistoryFailure) {
        ...
        if (fallbackMessages.length > 0) {
          ...
          applyLoadFailure(String(err), isBenignHistoryLoadFailure(err));
        }
      }
    })();
