...
import { getOpenClawConfigDir, getOpenClawDir, getOpenClawEntryPath, getOpenClawHomeDir, isOpenClawPresent } from '../utils/paths';
...
function cleanupStaleBuiltInExtensions(): void {
  ...
  for (const ext of BUILTIN_CHANNEL_EXTENSIONS) {
    const extDir = join(openclawConfigDir, 'extensions', ext);
...
function ensureConfiguredPluginsUpgraded(configuredChannels: string[]): void {
  ...
  for (const channelType of configuredChannels) {
    ...
    const targetDir = join(openclawConfigDir, 'extensions', dirName);
