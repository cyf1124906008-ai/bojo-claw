...
async function resolveFeishuPluginId(): Promise<string> {
    const extensionRoot = join(OPENCLAW_DIR, 'extensions');
...
export async function saveChannelConfig(
    ...
    return withConfigLock(async () => {
        ...
        if (!isBajoSupportedChannelType(resolvedChannelType)) {
            throw new Error(`Channel "${channelType}" is not supported by BajoClaw. Supported channels: BajoSeek, WeChat.`);
        }
...
export async function validateChannelCredentials(
    ...
    switch (resolveStoredChannelType(channelType)) {
        case BAJOSEEK_PLUGIN_ID:
            return validateBajoSeekCredentials(config);
        case 'discord':
...
async function validateBajoSeekCredentials(
    config: Record<string, string>
): Promise<CredentialValidationResult> {
    ...
    if (!botId) {
        return { valid: false, errors: ['请输入 BajoSeek Bot ID'], warnings: [] };
    }
    if (!token) {
        return { valid: false, errors: ['请输入 BajoSeek Token'], warnings: [] };
    }
    if (!/^wss?:\/\//i.test(wsUrl)) {
        return { valid: false, errors: ['WebSocket 地址必须以 ws:// 或 wss:// 开头'], warnings: [] };
    }

    const endpoint = `${wsUrl}/ws/bot`;
    return await new Promise<CredentialValidationResult>((resolve) => {
        let settled = false;
        let ws: WebSocket | null = null;
        const finish = (result: CredentialValidationResult) => {
            if (settled) return;
            settled = true;
            clearTimeout(timer);
            try { ws?.close(); } catch {
                // ignore close errors after validation settles
            }
            resolve(result);
        };
        ...
        const timer = setTimeout(() => {
            finish({
                valid: false,
                errors: [`连接 BajoSeek WebSocket 超时：${endpoint}`],
                warnings: [],
            });
        ...
        try {
            ...
            finish({
                valid: false,
                errors: [`创建 WebSocket 连接失败：${error instanceof Error ? error.message : String(error)}`],
                warnings: [],
            });
            return;
        }
        ...
        ws.on('open', () => {
            finish({
                valid: true,
                errors: [],
                warnings: [`BajoSeek WebSocket 连接验证通过：${wsUrl}`],
                details: { botId, wsUrl },
            });
        });
        ...
        ws.on('unexpected-response', (_request, response) => {
            ...
            if (status === 401 || status === 403) {
                finish({
                    valid: false,
                    errors: [`认证失败（HTTP ${status}）：请检查 Bot ID 和 Token`],
                    warnings: [],
                });
                return;
            }
            finish({
                valid: false,
                errors: [`BajoSeek WebSocket 握手失败（HTTP ${status}）：请确认 WebSocket 地址是否正确：${endpoint}`],
                warnings: [],
            });
        });
        ...
        ws.on('error', (error) => {
            ...
            const dnsHint = /ENOTFOUND|getaddrinfo/i.test(message)
                ? '域名解析失败，请确认 WebSocket 域名是否已配置。'
                : '请确认网络、代理或 WebSocket 服务是否可访问。';
            finish({
                valid: false,
                errors: [`BajoSeek WebSocket 连接失败：${message}。${dnsHint}`],
                warnings: [],
            });
        });
        ...
        ws.on('close', () => {
            if (!settled) {
                finish({
                    valid: false,
                    errors: ['BajoSeek WebSocket 在验证完成前关闭连接'],
                    warnings: [],
                });
            }
        });
