...
export function fixupPluginManifest(targetDir: string): void {
  ...
  patchBajoSeekDefaultWsUrl(targetDir);
}
...
const PLUGIN_NPM_NAMES: Record<string, string> = {
  ...
  bajoseek: '@bajoseek/openclaw-bajoseek',
};

function shouldBundlePeerDependencies(npmName: string): boolean {
  return npmName === '@bajoseek/openclaw-bajoseek';
}

function patchBajoSeekDefaultWsUrl(targetDir: string): void {
  const pkgPath = join(targetDir, 'package.json');
  let pkg: Record<string, unknown>;
  try {
    pkg = JSON.parse(readFileSync(fsPath(pkgPath), 'utf-8'));
  ...
  if (pkg.name !== '@bajoseek/openclaw-bajoseek') return;

  for (const rel of ['src/config.ts', 'src/config.js', 'dist/config.js']) {
    const filePath = join(targetDir, rel);
    if (!existsSync(fsPath(filePath))) continue;
    try {
      const original = readFileSync(fsPath(filePath), 'utf-8');
      const patched = original.replaceAll('ws://14.103.246.65:19090', 'wss://ws.bajoseek.com');
      if (patched !== original) {
        writeFileSync(fsPath(filePath), patched, 'utf-8');
        logger.info(`[plugin] Patched BajoSeek default WebSocket URL in ${rel}`);
      }
    } catch {
...
function shouldWriteNativeInstallMetadata(pluginDirName: string): boolean {
  return pluginDirName === 'bajoseek';
}

function shouldRepairBundledInstall(pluginDirName: string, targetDir: string): boolean {
  if (pluginDirName !== 'bajoseek') return false;

  const requiredDeps = [
    join(targetDir, 'node_modules', 'openclaw'),
    join(targetDir, 'node_modules', 'jiti'),
    join(targetDir, 'node_modules', '@modelcontextprotocol'),
    join(targetDir, 'node_modules', '@agentclientprotocol'),
  ];
  return requiredDeps.some((depPath) => !existsSync(fsPath(depPath)));
}

function writeNativeInstallMetadata(pluginDirName: string, targetDir: string): void {
  if (!shouldWriteNativeInstallMetadata(pluginDirName)) return;
  ...
  try {
    const pkg = JSON.parse(readFileSync(fsPath(pkgPath), 'utf-8')) as { version?: unknown };
    if (typeof pkg.version === 'string' && pkg.version.trim()) {
      packageVersion = pkg.version.trim();
    }
  } catch {
    // Metadata is best-effort; missing package.json should not block startup.
  }

  let manifestId = pluginDirName;
  try {
    const manifest = JSON.parse(readFileSync(fsPath(manifestPath), 'utf-8')) as { id?: unknown };
    if (typeof manifest.id === 'string' && manifest.id.trim()) {
      manifestId = manifest.id.trim();
    }
  } catch {
    // Use the plugin directory name fallback.
  }

  try {
    mkdirSync(fsPath(path.dirname(configPath)), { recursive: true });
    const existing = existsSync(fsPath(configPath))
      ? JSON.parse(readFileSync(fsPath(configPath), 'utf-8')) as Record<string, any>
      : {};

    const plugins =
      existing.plugins && typeof existing.plugins === 'object'
        ? existing.plugins as Record<string, any>
        : {};
    const entries =
    ...
    const installs =
      plugins.installs && typeof plugins.installs === 'object'
        ? plugins.installs as Record<string, any>
        : {};
    const previousInstall =
      installs[manifestId] && typeof installs[manifestId] === 'object'
        ? installs[manifestId] as Record<string, any>
        : {};

    entries[manifestId] = {
      ...(entries[manifestId] && typeof entries[manifestId] === 'object' ? entries[manifestId] : {}),
      enabled: true,
    };
    ...
    installs[manifestId] = {
      ...
      ...(packageVersion
        ? {
          ...
          resolvedSpec: `${npmName}@${packageVersion}`,
        }
        : {}),
      installedAt: typeof previousInstall.installedAt === 'string'
    ...
    writeFileSync(fsPath(configPath), JSON.stringify(existing, null, 2) + '\n', 'utf-8');
    logger.info(`[plugin] Wrote native install metadata for ${manifestId}`);
  } catch (error) {
    logger.warn(`[plugin] Failed to write native install metadata for ${pluginDirName}`, error);
  }
}
...
export function copyPluginFromNodeModules(npmPkgPath: string, targetDir: string, npmName: string): void {
  ...
  try {
    ...
    if (!shouldBundlePeerDependencies(npmName)) {
...
export function ensurePluginInstalled(
  ...
  const targetDir = join(openclawConfigDir, 'extensions', pluginDirName);
  ...
  if (existsSync(fsPath(targetManifest))) {
    if (!sourceDir) {
      writeNativeInstallMetadata(pluginDirName, targetDir);
    ...
    if (!sourceVersion || !installedVersion || sourceVersion === installedVersion) {
      if (shouldRepairBundledInstall(pluginDirName, targetDir)) {
        logger.info(`[plugin] Repairing ${pluginLabel} plugin install: bundled peer dependencies are missing`);
      } else {
        writeNativeInstallMetadata(pluginDirName, targetDir);
  ...
  if (sourceDir) {
    const extensionsRoot = join(openclawConfigDir, 'extensions');
    ...
    if (npmName) {
      ...
      if (existsSync(fsPath(join(npmPkgPath, 'openclaw.plugin.json')))) {
        ...
        if (sourceVersion && (!installedVersion || sourceVersion !== installedVersion)) {
          ...
          try {
            mkdirSync(fsPath(join(openclawConfigDir, 'extensions')), { recursive: true });
...
export function ensureBajoSeekPluginInstalled(): { installed: boolean; warning?: string } {
  return ensurePluginInstalled('bajoseek', buildCandidateSources('bajoseek'), 'BajoSeek');
}
...
const ALL_BUNDLED_PLUGINS = [
  ...
  { fn: ensureBajoSeekPluginInstalled, label: 'BajoSeek' },
] as const;
