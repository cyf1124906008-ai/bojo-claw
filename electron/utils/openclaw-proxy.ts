...
interface SyncProxyOptions {
   ...
   * currently disabled in BajoClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync BajoClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because BajoClaw proxy is disabled and preserve mode is enabled');
      return;
    }
