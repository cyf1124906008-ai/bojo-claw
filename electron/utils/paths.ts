...
export function expandPath(path: string): string {
  if (path.startsWith('~')) {
    return path.replace('~', getOpenClawHomeDir());
  }
  return path;
}
 ...
 *
 * Keeping this under BajoClaw's userData directory prevents BajoClaw from
...
export function getClawHubCliEntryPath(): string {
  if (getElectronApp().isPackaged) {
    return join(process.resourcesPath, 'clawhub', 'node_modules', 'clawhub', 'bin', 'clawdhub.js');
  }
