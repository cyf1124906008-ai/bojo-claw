 ...
 * BajoClaw intentionally avoids the legacy ClawX OSS feed so packaged builds
...
/** Optional BajoClaw update feed. Disabled unless explicitly configured. */
...
export class AppUpdater extends EventEmitter {
  ...
  private setupListeners(): void {
    ...
    autoUpdater.on('update-available', (info: UpdateInfo) => {
      if (!this.isAcceptableUpdate(info)) {
  ...
  private isAcceptableUpdate(info: UpdateInfo): boolean {
    const currentVersion = app.getVersion();
    if (compareVersions(info.version, currentVersion) <= 0) {
      logger.warn(
        `[Updater] Ignoring non-newer update candidate ${info.version}; current version is ${currentVersion}.`
      );
      return false;
    }
  ...
  async checkForUpdates(): Promise<UpdateInfo | null> {
    try {
      if (!BAJO_UPDATES_ENABLED) {
        logger.info('[Updater] Update checking disabled; no BajoClaw update feed configured.');
        this.updateStatus({ status: 'not-available' });
        return null;
      }
      ...
      if (result.updateInfo && !this.isAcceptableUpdate(result.updateInfo)) {
        this.updateStatus({ status: 'not-available' });
        return null;
      }
