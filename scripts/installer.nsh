; BajoClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customHeader
  ...
  !define MUI_LICENSEPAGE_TEXT_TOP "安装 BajoClaw 前，请阅读许可条款。"
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting BajoClaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers BajoClaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if BajoClaw.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old BajoClaw was installed
  ...
  ; "Can't modify BajoClaw's files" retry loop -> "BajoClaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "BajoClaw 无法关闭" dialog.
...
; "BajoClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with BajoClaw's
...
!macro customUnInstall
  ...
  _cu_removeData:
    ...
    RMDir /r "$LOCALAPPDATA\bajo-claw"
    RMDir /r "$APPDATA\bajo-claw"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\bajo-claw
    IfFileExists "$LOCALAPPDATA\bajo-claw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\bajo-claw"
      IfFileExists "$LOCALAPPDATA\bajo-claw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\bajo-claw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\bajo-claw
    IfFileExists "$APPDATA\bajo-claw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\bajo-claw"
      IfFileExists "$APPDATA\bajo-claw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\bajo-claw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\bajo-claw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\bajo-claw"
    IfFileExists "$APPDATA\bajo-claw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\bajo-claw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\bajo-claw"
    RMDir /r "$R3\AppData\Roaming\bajo-claw"
