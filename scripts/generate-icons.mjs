...
try {
  ...
  const TRAY_PNG_SOURCE = path.join(ICONS_DIR, 'tray-icon-source.png');
  const TRAY_SVG_SOURCE = path.join(ICONS_DIR, 'tray-icon-template.svg');
  const TRAY_SOURCE = fs.existsSync(TRAY_PNG_SOURCE) ? TRAY_PNG_SOURCE : TRAY_SVG_SOURCE;
  
  if (fs.existsSync(TRAY_SOURCE)) {
    await sharp(TRAY_SOURCE)
      .resize(22, 22)
      .png()
