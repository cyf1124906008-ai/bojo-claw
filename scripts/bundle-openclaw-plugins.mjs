 ...
 *   - @bajoseek/openclaw-bajoseek -> build/openclaw-plugins/bajoseek
 *
...
function cpSyncSafe(src, dest) {
  ...
  copyDirSyncRecursive(normWin(src), normWin(dest));
}
...
function copyDirSyncRecursive(src, dest) {
  ...
  for (const entry of entries) {
    ...
    const info = fs.statSync(srcChild);
    if (info.isDirectory()) {
      copyDirSyncRecursive(srcChild, destChild);
    } else {
...
function shouldBundlePeerDependencies(pluginId) {
...
function bundleOnePlugin({ npmName, pluginId }) {
  ...
  cpSyncSafe(realPluginPath, outputDir);
  ...
  try {
    ...
    if (!shouldBundlePeerDependencies(pluginId)) {
  ...
  patchBajoSeekDefaultWsUrl(outputDir, pluginId);
...
function patchBajoSeekDefaultWsUrl(pluginDir, pluginId) {
  ...
  const targetFiles = [
    path.join(pluginDir, 'src', 'config.ts'),
    path.join(pluginDir, 'src', 'config.js'),
    path.join(pluginDir, 'dist', 'config.js'),
  ];
  ...
  for (const file of targetFiles) {
    ...
    if (patched !== original) {
      ...
      echo`   🩹 Patched BajoSeek default WebSocket URL in ${path.relative(pluginDir, file)}`;
    }
  }
}
